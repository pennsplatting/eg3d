...
def render(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, override_color = None):
    ...
    if override_color is None:
        if pipe.convert_SHs_python:       
            ...
            st()
        # shs = pc.get_features # should map to ([0,1]=0.5)/C0
        # print(f"scaling:{scales.shape}, rotations{rotations.shape}, shs{shs.shape if shs is not None else None}, colors_precomp{colors_precomp.shape if colors_precomp is not None else None}")
    else:
        st()
