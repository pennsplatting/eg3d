...
def batch_render(viewpoint_camera_list, pc, pipe, bg_color : torch.Tensor,
    ...
    print("world_view_transform_list", world_view_transform_list)
    print("full_proj_transform_list", full_proj_transform_list)
    print("camera_center_list", camera_center_list)
    ...
    print(rendered_image.shape, rendered_image.device)
    # Those Gaussians that were frustum culled or had a radius of 0 were not visible.
    # They will be excluded from value updates used in the splitting criteria.
    return {"render": rendered_image,
            "viewspace_points": screenspace_points,
